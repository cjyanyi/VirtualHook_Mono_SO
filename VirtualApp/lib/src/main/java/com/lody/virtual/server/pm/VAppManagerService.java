...
//log
        VLog.w(TAG, "NOTE THE FAKE ADDRESS: " + libDir.getAbsolutePath());
        VLog.w(TAG, "NOTE THE FAKE ADDRESS: " + libDir.getPath());
        //VLog.i(TAG, "MONO:-- " + VEnvironment.getDataDirectory().getPath());

        VLog.i(TAG, "MONO1:-- "+ pkg.packageName);
        VLog.i(TAG, "MONO2:--" + new File(path).getAbsolutePath());
        ...
        try {
            ...
            if(Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
                ...
                if (!vaNativeSo.exists()) {
                   ...
                   // VLog.i(TAG, "MONO3:--" + new File(VEnvironment.getRoot().getParent()).getAbsolutePath());
                }
                File vaMonoSo = new File(libDir, "libmono.so");
                File vaPlugin = new File(appDir.getParent(), "lab.galaxy.demeHookPlugin/lib/libmono.so");
                if (vaMonoSo.exists()) {
                    if (!vaPlugin.exists()) {
                        FileUtils.createSymlink(
                                vaMonoSo.getAbsolutePath(),
                                vaPlugin.getAbsolutePath());
                    }
                    VLog.i(TAG, "MONO4:--" + vaPlugin.getAbsolutePath());
